...
test('browser icon assets use compatible formats and bounded payloads', () => {
  const favicon32 = statSync(`${webRoot}public/icons/app-favicon-32.png`);
  const appleTouch = statSync(`${webRoot}public/icons/app-apple-touch-icon.png`);
  const headerLogo = statSync(`${webRoot}public/icons/app-logo-256.png`);
