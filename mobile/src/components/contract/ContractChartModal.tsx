...
import MobileKlineChart from '../trade/MobileKlineChart';
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <View style={styles.safe}>
        ...
        <ScrollView
          ...
          <Text style={styles.chartTitle}>{symbolLabel} 合约 K线</Text>
          <MobileKlineChart
            error={error}
            height={320}
            interval={interval}
              ...
              {trades.slice(0, 8).map(item => (
                <View key={item.id} style={styles.row}>
                  <Text
                    ...
                    {formatContractNumber(item.price, pricePrecision)}
                  </Text>
                  <Text style={styles.rowAmount}>
                    {formatContractNumber(item.amount, 4)}
                  </Text>
                </View>
              ))}
            </Panel>
...
export default React.memo(ContractChartModal);
...
function Level({
  ...
  color: string;
  item: ContractOrderBookLevel;
  pricePrecision: number;
}) {
  return (
    <View style={styles.row}>
      <Text style={[styles.rowPrice, {color}]}>
        {formatContractNumber(item.price, pricePrecision)}
      </Text>
      <Text style={styles.rowAmount}>{formatContractNumber(item.amount, 4)}</Text>
    </View>
  );
}
