...
_spot_depth_market_cache = SharedMarketCacheAdapter(
    l1_ttl_ms=SPOT_DEPTH_SHARED_CACHE_L1_TTL_MS,
)


@dataclass(frozen=True)
class MarketDepthCacheMetadata:
    data: dict[str, Any]
    source: str
    provider: str
    updated_at: int
    ...
    def to_domain_snapshot(
        ...
        *,
        ...
        return build_market_domain_snapshot(
            ...
            updated_at=self.updated_at,
            version=self.version,
            max_age_ms=SPOT_DEPTH_SHARED_CACHE_TTL_MS,
...
def get_spot_depth_with_shared_cache(
    *,
    ...
    adapter = cache_adapter or _spot_depth_market_cache
    ...
    if cache_hit is not None:
        if cache_hit_observer is not None:
            try:
                ...
                logger.debug(
                    "spot_depth_market_cache_observer_failed key=%s reason=%s",
                    key,
                    exc,
                )
...
def _get_fresh_cached_depth(
    ...
    *,
    ...
    try:
        ...
        logger.debug("spot_depth_market_cache_get_failed key=%s reason=%s", key, exc)
        return None
    if envelope is None or envelope.freshness != FRESHNESS_FRESH:
        return None

    metadata = _metadata_from_envelope(envelope)
    if metadata is None or not _is_cacheable_spot_depth(metadata.data):
        return None
...
def _set_cached_depth(
    ...
    *,
    ...
    try:
        ...
        logger.debug("spot_depth_market_cache_set_failed key=%s reason=%s", key, exc)


def _metadata_from_payload(payload: Mapping[str, Any]) -> MarketDepthCacheMetadata:
    ...
    return MarketDepthCacheMetadata(
        data=dict(payload),
        source=source,
        provider=provider,
        updated_at=now_ms(),
        sequence=_non_negative_int(payload.get("sequence")),
    )


def _metadata_from_envelope(
    envelope: MarketCacheEnvelope,
) -> Optional[MarketDepthCacheMetadata]:
    raw = envelope.data
    ...
    if isinstance(nested_data, Mapping) and all(
        key in raw
        for key in ("source", "provider", "updated_at", "version", "sequence")
    ):
    ...
    # Compatibility with the pre-B-2.2 cache where envelope.data was raw depth.
