...
_spot_trades_shared_cache = SharedMarketCacheAdapter(
...
def get_spot_trades_with_shared_cache(
    *,
    ...
    adapter = cache_adapter or _spot_trades_shared_cache
    cached = _get_fresh_cached_trades(adapter, key=key, ttl_ms=ttl_ms)
...
def _get_fresh_cached_trades(
    ...
    *,
    ...
    try:
        ...
        logger.debug("spot_trades_shared_cache_get_failed key=%s reason=%s", key, exc)
        return None
    ...
    if not _is_cacheable_spot_trades(payload):
        return None
    return payload
...
def _set_cached_trades(
    ...
    *,
    ...
    try:
        ...
        logger.debug("spot_trades_shared_cache_set_failed key=%s reason=%s", key, exc)
