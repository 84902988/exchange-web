...
UPLOAD_ASSET_ICON_INPUT_MAX_BYTES = 5 * 1024 * 1024
UPLOAD_ASSET_ICON_OUTPUT_MAX_BYTES = 64 * 1024
UPLOAD_ASSET_ICON_SVG_MAX_BYTES = 64 * 1024
UPLOAD_IMAGE_TYPES = {
    "image/png": {".png"},
    "image/jpeg": {".jpg", ".jpeg"},
    "image/webp": {".webp"},
...
def _compress_asset_icon(content: bytes, target: Path) -> None:
    try:
        from PIL import Image, ImageOps, UnidentifiedImageError
    except ImportError as exc:
        raise HTTPException(status_code=500, detail="服务器未安装图片处理组件") from exc
    ...
    try:
        with Image.open(io.BytesIO(content)) as image:
            image = ImageOps.exif_transpose(image)
            resampling = getattr(getattr(Image, "Resampling", Image), "LANCZOS")
            image.thumbnail((160, 160), resampling)
            if image.mode not in {"RGB", "RGBA"}:
                has_alpha = image.mode in {"LA", "PA"} or (
                    image.mode == "P" and "transparency" in image.info
                )
        ...
        target.unlink(missing_ok=True)
        raise HTTPException(status_code=400, detail="图片处理失败，请更换文件后重试") from exc

    if target.stat().st_size <= UPLOAD_ASSET_ICON_OUTPUT_MAX_BYTES:
        return
    target.unlink(missing_ok=True)
    raise HTTPException(status_code=400, detail="图片压缩后仍超过 64KB，请降低清晰度或尺寸")
...
async def upload_asset_icon(
    ...
    content = await file.read(UPLOAD_ASSET_ICON_INPUT_MAX_BYTES + 1)
    await file.close()
    if not content:
        raise HTTPException(status_code=400, detail="Empty file")
    if len(content) > UPLOAD_ASSET_ICON_INPUT_MAX_BYTES:
        raise HTTPException(status_code=400, detail="Image must be 5MB or smaller; it will be compressed automatically")

    original_ext = Path(file.filename or "").suffix.lower()
    upload_dir = BASE_DIR / "static" / "uploads" / "assets"
    upload_dir.mkdir(parents=True, exist_ok=True)
    if content_type == "image/svg+xml":
        if len(content) > UPLOAD_ASSET_ICON_SVG_MAX_BYTES:
            raise HTTPException(status_code=400, detail="SVG icon must be 64KB or smaller")
        ...
        target = upload_dir / filename
        _compress_asset_icon(content, target)
...
def _contract_symbol_form_options() -> Dict[str, list[str]]:
    return {
        ...
        "holiday_calendar_code_options": ["", "US", "GB"],
        "session_profile_code_options": [
            "CRYPTO_24_7",
            "US_EQUITY",
            "US_INDEX_EXTENDED",
            "FOREX_24X5",
...
def contract_symbol_create_submit(
    ...
    holiday_calendar_code: str = Form(""),
    session_profile_code: str = Form(""),
    session_timezone_override: str = Form(""),
    extended_hours_execution_mode: str = Form("DISPLAY_ONLY"),
    price_precision: str = Form("8"),
