...
from app.services.spot_trades_shared_cache import (  # noqa: E402
...
def _trades(symbol: str = "BTCUSDT", price: str = "100", *, source: str | None = "LIVE_WS") -> dict[str, Any]:
    return {
        "symbol": symbol,
        "trades": [
            {
        ...
        "provider": "OKX_SPOT" if source != "internal" else None,
        "provider_symbol": "BTC-USDT" if source != "internal" else None,
        "stale": False,
...
def test_spot_trades_redis_hit_skips_loader_and_preserves_shape() -> None:
    key = spot_trades_shared_cache_key("BTCUSDT")
    clock = Clock(10_000)
    cached = make_market_cache_envelope(
        _trades(price="101", source="LIVE_WS"),
        source="LIVE_WS",
        provider="OKX_SPOT",
    ...
    assert result["trades"][0]["source"] == "LIVE_WS"
    assert result["trades"][0]["freshness"] == "LIVE"
    assert redis.get_calls == 1
    assert redis.set_calls == []


def test_spot_trades_l1_hit_skips_redis_and_loader() -> None:
    clock = Clock(10_000)
    redis = FakeRedis()
    adapter = _adapter(redis, clock, l1_ttl_ms=1000)

    first = get_spot_trades_with_shared_cache(
        symbol="BTCUSDT",
        data_source="BINANCE",
        loader=lambda: _trades(price="102"),
        cache_adapter=adapter,
    )
    assert first is not None
    assert first["trades"][0]["price"] == "102"

    redis.get_calls = 0
    redis.store[spot_trades_shared_cache_key("BTCUSDT")] = json.dumps(
        make_market_cache_envelope(
            _trades(price="999"),
            source="LIVE_WS",
            provider="OKX_SPOT",
    ...
    second = get_spot_trades_with_shared_cache(
        symbol="BTCUSDT",
        data_source="BINANCE",
        loader=lambda: (_ for _ in ()).throw(AssertionError("loader should not run")),
        cache_adapter=adapter,
    ...
    assert second["trades"][0]["price"] == "102"
    assert redis.get_calls == 0


def test_spot_trades_miss_loads_and_writes_cache() -> None:
    key = spot_trades_shared_cache_key("BTCUSDT")
    clock = Clock(10_000)
    redis = FakeRedis()
    adapter = _adapter(redis, clock)
    ...
    result = get_spot_trades_with_shared_cache(
        symbol="BTCUSDT",
        data_source="BINANCE",
        loader=lambda: _trades(price="103"),
        cache_adapter=adapter,
    )

    assert result is not None
    assert result["trades"][0]["price"] == "103"
    assert result["provider"] == "OKX_SPOT"
    assert result["source"] == "LIVE_WS"
    assert result["freshness"] == "LIVE"
    assert redis.set_calls and redis.set_calls[0][0] == key
    stored = json.loads(redis.store[key])
    assert stored["data"]["trades"][0]["price"] == "103"
    assert stored["source"] == "LIVE_WS"
    assert stored["provider"] == "OKX_SPOT"
    assert stored["ttl_ms"] == SPOT_TRADES_SHARED_CACHE_TTL_MS


def test_spot_trades_redis_down_falls_back_to_loader() -> None:
    clock = Clock(10_000)
    ...
    result = get_spot_trades_with_shared_cache(
        symbol="BTCUSDT",
        data_source="BINANCE",
        loader=lambda: _trades(price="104"),
        cache_adapter=adapter,
    )

    assert result is not None
    assert result["trades"][0]["price"] == "104"
    assert redis.get_calls == 1


def test_spot_trades_stale_cache_calls_loader_and_refreshes() -> None:
    key = spot_trades_shared_cache_key("BTCUSDT")
    clock = Clock(10_000)
    cached = make_market_cache_envelope(
        _trades(price="105"),
    ...
    result = get_spot_trades_with_shared_cache(
        symbol="BTCUSDT",
        data_source="BINANCE",
        loader=lambda: _trades(price="106"),
        cache_adapter=adapter,
        ttl_ms=1000,
    )

    assert result is not None
    assert result["trades"][0]["price"] == "106"
    assert redis.set_calls and redis.set_calls[-1][0] == key
    stored = json.loads(redis.store[key])
    assert stored["data"]["trades"][0]["price"] == "106"


def test_internal_symbol_does_not_read_or_write_external_trades_cache() -> None:
    key = spot_trades_shared_cache_key("MFCUSDT")
    clock = Clock(10_000)
    cached = make_market_cache_envelope(
        _trades("MFCUSDT", price="999"),
        source="LIVE_WS",
        provider="OKX_SPOT",
        ttl_ms=SPOT_TRADES_SHARED_CACHE_TTL_MS,
        updated_at_ms=clock(),
    ...
    result = get_spot_trades_with_shared_cache(
        ...
        loader=lambda: _trades("MFCUSDT", price="10", source="internal"),
        cache_adapter=adapter,
    )
